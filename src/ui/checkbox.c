static int checkbox_paint_on_off(view_t* ui) {
...
static const char*  checkbox_on_off_label(view_t* ui, char* label, int32_t count)  {
...
static void  checkbox_measure(view_t* ui) {
    assert(ui->tag == uic_tag_checkbox);
    view_measure(ui);
    ui->w += ui->em.x * 2;
}

static void  checkbox_paint(view_t* ui) {
    assert(ui->tag == uic_tag_checkbox);
    char text[countof(ui->text)];
    const char* label =  checkbox_on_off_label(ui, text, countof(text));
    gdi.push(ui->x, ui->y);
    font_t f = ui->font != null ? *ui->font : app.fonts.regular;
    font_t font = gdi.set_font(f);
    gdi.x =  checkbox_paint_on_off(ui) + ui->em.x * 3 / 4;
    gdi.text("%s", label);
    gdi.set_font(font);
    gdi.pop();
}
...
static void  checkbox_character(view_t* ui, const char* utf8) {
    assert(ui->tag == uic_tag_checkbox);
    assert(!ui->hidden && !ui->disabled);
    char ch = utf8[0];
    if (uic_is_keyboard_shortcut(ui, ch)) {
         checkbox_flip((checkbox_t*)ui);
    }
}

static void checkbox_key_pressed(view_t* ui, int32_t key) {
...
static void  checkbox_mouse(view_t* ui, int32_t message, int32_t flags) {
    assert(ui->tag == uic_tag_checkbox);
    (void)flags; // unused
    assert(!ui->hidden && !ui->disabled);
...
void _checkbox_init_(view_t* ui) {
    assert(ui->tag == uic_tag_checkbox);
    view_init(ui);
    view_set_text(ui, ui->text);
    ui->mouse       =  checkbox_mouse;
    ui->measure     = checkbox_measure;
    ui->paint       = checkbox_paint;
    ui->character   = checkbox_character;
    ui->key_pressed = checkbox_key_pressed;
    ui->localize(ui);
